...
router.get('/list', (req, res) => {
    ...
    SQL((error, connection) => {
        ...
        let sql = `select idx,subject,userid,content,hit,date_format(today,'%Y-%m-%d')as today,number from boardtb order by idx desc `;
...
router.post('/write', (req, res) => {
    ...
    SQL((error, connection) => {
        ...
        let sql = `insert into boardtb (subject,userid,content)
                values('${body.subject}','${body.userid}','${body.content}')`;
        connection.query(sql, (error, results) => {
            if (error) {
                console.log(error);
            } else {
...
router.get('/view', (req, res) => {
    ...
    SQL((error, connection) => {
        ...
        let sql0 = `select idx,number from boardtb order by idx desc `;
        let page;
        connection.query(sql0, (error, results) => {
            if (error) {
                ...
                results.forEach(ele => {
                    ele.number = total_record--;
                    if (ele.idx == idx) {
                        page = Math.floor((results.length - ele.number) / articleCount) + 1;
                    }
                });

            }
        })
        let sql = `update boardtb set hit=hit+1 where idx=${idx};`
        connection.query(sql, (error, results) => {
            if (error) {
                console.log(error);
            } else {
                let sql = `select  idx,subject,userid,content,hit,date_format(today,'%Y-%m-%d') 
                        as today from boardtb where idx=${idx};`
                connection.query(sql, (error, results) => {
                    if (error) {
                        console.log(error);
                    } else {
...
router.get('/modify', (req, res) => {
    ...
    SQL((error, connection) => {
        ...
        let sql = `select idx, subject,userid,content from boardtb where idx=${idx};`
        connection.query(sql, (error, results) => {
            if (error) {
                console.log(error);
            } else {
...
router.post('/modify', (req, res) => {
    ...
    SQL((error, connection) => {
        ...
        let sql = `update boardtb set subject='${body.subject}', userid='${body.userid}', content='${body.content}' where idx=${body.idx};`
        connection.query(sql, (error, results) => {
            if (error) {
                console.log(error);
            } else {
...
router.get('/delete', (req, res) => {
    ...
    SQL((error, connection) => {
        ...
        let sql = `delete from boardtb where idx=${idx};`
        connection.query(sql, (error, results) => {
            if (error) {
                console.log(error);
            } else {
